...
TicTacToe.prototype.setPlayers = function (name, symbol) {
  ...
  this.player2 = new player(
    "computer",
    this.player1.symbol.toUpperCase() == "X" ? "O" : "X"
  );
...
TicTacToe.prototype.checkWinner = function () {
  ...
  //checks if there is a diagonal line of 3 of one's own symbol
  if (
    this.board[row + 2][col] === this.board[row + 1][col + 1] &&
    ...
    for (var row = 0; row < 3; row++) {
      for (var col = 0; col < 3; col++) {
        if (this.board[row][col] === 0) {
          ...
          bestVal = Math.max(
            bestVal,
            this.minimax(board, depth + 1, !isMaximizingPlayer)
          );
          this.board[row][col] = 0;
          this.winningMoves = null;
        }
...
var view = {
    ...
    boxList.addEventListener("click", function (event) {
      if (view.boardEnabled) {
        ...
        controller.playerTurn(
          view.btnEntry[boxClicked.id][0],
          view.btnEntry[boxClicked.id][1]
        );
      }
    });
  ...
  setScoreBoard: function (player1, player2) {
    var player1Color =
      player1.symbol.toUpperCase() == "X"
        ? "x-gradient-color"
        : "o-gradient-color";
    var player2Color =
      player2.symbol.toUpperCase() == "X"
        ? "x-gradient-color"
        : "o-gradient-color";
  ...
  showWinningMove: function (winningMove) {
    if (winningMove == null) {
      ...
      for (var i = 0; i < winningMove.length; i++) {
        for (prop in this.btnEntry) {
          if (
            this.btnEntry[prop][0] == winningMove[i][0] &&
            this.btnEntry[prop][1] == winningMove[i][1]
          ) {
